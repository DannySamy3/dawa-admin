...
import { Eye, Search, FileText, RefreshCw, Package, CheckCircle, XCircle, AlertCircle } from 'lucide-react';
import { AdminLayout } from '@/components/layout/AdminLayout';
import { Pagination } from '@/components/ui/Pagination';
import { Modal } from '@/components/ui/Modal';
import { verifyBadge } from '@/components/ui/Badge';
import { adminApi } from '@/lib/api';
import type { ImporterProfile } from '@/lib/types';

type Tab = 'all' | 'pending' | 'approved' | 'rejected';

export default function ImportersPage() {
  const [items, setItems] = useState<ImporterProfile[]>([]);
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  const [total, setTotal] = useState(0);
  const [search, setSearch] = useState('');
  const [selected, setSelected] = useState<ImporterProfile | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await adminApi.getImporters(page, LIMIT);
      const list: ImporterProfile[] = data.data ?? data.importers ?? data ?? [];
  ...
  const handleVerify = async (userId: string, status: 'APPROVED' | 'REJECTED') => {
    ...
    try {
      ...
      showToast(`Importer ${status === 'APPROVED' ? 'approved' : 'rejected'} successfully`, 'success');
      setSelected(null);
      load();
    ...
    <AdminLayout>
      ...
      <div className="page-header">
        <div className="page-header-left">
          <div className="page-title">Importers</div>
          <div className="page-subtitle">{total} importer companies registered</div>
        </div>
        ...
        <table className="table">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5}><div className="empty-state"><Package size={36}/><p>No importers found</p></div></td></tr>
            ) : filtered.map((imp) => (
              <tr key={imp.id}>
                <td>
                  <div style={{ display:'flex',alignItems:'center',gap:10 }}>
                    ...
                    <span className="table-name">{imp.companyName}</span>
                  </div>
                </td>
                <td><span className="table-email">{imp.user?.email ?? '—'}</span></td>
                <td style={{ fontSize:12,color:'var(--text-muted)' }}>{imp.locationPremises ?? imp.city ?? '—'}</td>
                <td>{verifyBadge(imp.status || 'PENDING')}</td>
                <td>
                  <button className="btn btn-secondary btn-sm btn-icon" onClick={() => setSelected(imp)}>
                    <Eye size={14} />
                  </button>
                </td>
              </tr>
      ...
      <Modal title="Importer Profile & Verification" open={!!selected} onClose={() => setSelected(null)}
        footer={
        ...
        {selected && (
          <div style={{ display:'flex',flexDirection:'column',gap:18 }}>
            ...
            <div className="detail-grid">
              <div className="detail-row"><div className="detail-label">Location</div><div className="detail-value">{selected.locationPremises ?? '—'}</div></div>
            ...
            {selected.md && (
              <>
                <div style={{ fontWeight:600,color:'var(--text-primary)',fontSize:14, marginTop: 8 }}>Managing Director</div>
                <div className="detail-grid">
                  <div className="detail-row"><div className="detail-label">Name</div><div className="detail-value">{selected.md.firstName} {selected.md.lastName}</div></div>
                  <div className="detail-row"><div className="detail-label">Email</div><div className="detail-value">{selected.md.email}</div></div>
                  <div className="detail-row"><div className="detail-label">Phone</div><div className="detail-value">{selected.md.phone ?? '—'}</div></div>
                </div>
              </>
            )}
