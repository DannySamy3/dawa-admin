...
import { Eye, Search, FileText, RefreshCw, Building2, CheckCircle, XCircle, AlertCircle } from 'lucide-react';
import { AdminLayout } from '@/components/layout/AdminLayout';
import { Pagination } from '@/components/ui/Pagination';
import { Modal } from '@/components/ui/Modal';
import { Badge, verifyBadge } from '@/components/ui/Badge';
import { adminApi } from '@/lib/api';
import type { InstitutionProfile, InstitutionType } from '@/lib/types';

const INST_TYPES: ('ALL' | InstitutionType)[] = ['ALL', 'PHARMACY', 'DISPENSARY', 'CLINIC', 'HOSPITAL', 'OTHER'];
...
export default function InstitutionsPage() {
  ...
  const [selected, setSelected] = useState<InstitutionProfile | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await adminApi.getInstitutions(page, LIMIT);
      const list: InstitutionProfile[] = data.data ?? data.institutions ?? data ?? [];
  ...
  const handleVerify = async (userId: string, status: 'APPROVED' | 'REJECTED') => {
    ...
    try {
      ...
      showToast(`Institution ${status === 'APPROVED' ? 'approved' : 'rejected'} successfully`, 'success');
      setSelected(null);
      load();
    ...
    <AdminLayout>
      ...
      <div className="page-header">
        <div className="page-header-left">
          ...
          <div className="page-subtitle">{total} institutions registered on the platform</div>
        </div>
      ...
      <div className="filters-bar" style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <select className="filter-select" value={typeFilter} onChange={(e) => setTypeFilter(e.target.value as 'ALL' | InstitutionType)}>
          {INST_TYPES.map((t) => <option key={t} value={t}>{t === 'ALL' ? 'All Types' : t}</option>)}
        </select>
        ...
        <table className="table">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5}><div className="empty-state"><Building2 size={36}/><p>No institutions found</p></div></td></tr>
            ) : filtered.map((inst) => (
              <tr key={inst.id}>
                <td>
                  <div style={{ display:'flex',alignItems:'center',gap:10 }}>
                    <div style={{ width:34,height:34,borderRadius:8,background:`${typeColor[inst.type] ?? '#94a3b8'}22`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0 }}>
                      <Building2 size={16} color={typeColor[inst.type] ?? '#94a3b8'}/>
                    </div>
                    <span className="table-name">{inst.institutionName}</span>
                  </div>
                </td>
                <td><span className="table-email">{inst.user?.email ?? '—'}</span></td>
                <td><Badge variant="info">{inst.type}</Badge></td>
                <td>{verifyBadge(inst.status || 'PENDING')}</td>
                <td>
                  <button className="btn btn-secondary btn-sm btn-icon" onClick={() => setSelected(inst)}><Eye size={14}/></button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <Modal title="Institution Profile & Verification" open={!!selected} onClose={() => setSelected(null)}
        footer={
        ...
        {selected && (
          <div style={{ display:'flex',flexDirection:'column',gap:18 }}>
            <div style={{ display:'flex',alignItems:'center',gap:14, paddingBottom: 16, borderBottom: '1px solid var(--border-subtle)' }}>
              <div style={{ width:52,height:52,borderRadius:12,background:`${typeColor[selected.type] ?? '#94a3b8'}22`,display:'flex',alignItems:'center',justifyContent:'center' }}>
                <Building2 size={24} color={typeColor[selected.type] ?? '#94a3b8'}/>
              </div>
              <div>
                <div style={{ fontSize:18,fontWeight:700,color:'var(--text-primary)' }}>{selected.institutionName}</div>
            ...
            <div className="detail-grid">
              ...
              <div className="detail-row"><div className="detail-label">Registered</div><div className="detail-value">{new Date(selected.createdAt).toLocaleDateString()}</div></div>
            </div>
